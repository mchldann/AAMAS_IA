...
public class XMLGenerator
{
	...
	{	
		...
		double sy_pr_goal_has_single_plan = SynthGenerator.def_pr_goal_has_single_plan;
			...
			case "synth": // Synthetic Generator
				{
					...
					while(i < args.length && args[i].startsWith("-"))
					{
						...
						switch (flag)
						{
							...
							case 'z': // probability of a goal having only one plan
								try
								{
									sy_pr_goal_has_single_plan = Double.parseDouble(args[i++]);
									if (sy_pr_goal_has_single_plan < 0 || sy_pr_goal_has_single_plan > 1){
										throw (new IllegalArgumentException());
									}
									break;
								}catch(Exception e)
								{
									System.out.println("The probability of a goal having only one plan must be >= 0 and <= 1");
									System.exit(1);
								}
					...
					gen = new SynthGenerator(seed, sy_depth, num_tree, sy_num_goal, sy_num_plan, sy_num_action, sy_num_var, sy_safety_factor, sy_pr_goal_has_single_plan);
					break;
				}
